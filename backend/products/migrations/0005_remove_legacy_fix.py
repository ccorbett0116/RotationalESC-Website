...
def fix_orphaned_data(apps, schema_editor):
    ...
    # Use raw SQL to get orphaned specs to avoid FK constraint issues
    with connection.cursor() as cursor:
        cursor.execute("""
            SELECT DISTINCT product_id 
            FROM products_productspecification 
            WHERE product_id NOT IN (
                SELECT id FROM products_product
            )
        """)
        for row in cursor.fetchall():
            orphaned_spec_product_ids.add(row[0])
    ...
    with connection.cursor() as cursor:
        ...
        """)
        for row in cursor.fetchall():
            orphaned_image_product_ids.add(row[0])
        ...
        try:
            ...
            if isinstance(old_product_id, str):
                try:
                    ...
                    # If string is not a valid UUID, generate one based on hash
                    product_uuid = uuid.uuid5(uuid.NAMESPACE_DNS, str(old_product_id))
            else:
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('products', '0004_section_manufacturer'),
    ]
    ...
    operations = [
        migrations.RunPython(
            ...
            atomic=False  # Disable atomic operation to allow PRAGMA commands
        ),
    ]
